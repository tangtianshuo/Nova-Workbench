...
- [ ] **PERSIST-08**: 修复 `rndStore` 的 `INITIAL.p1` fallback bug(CONCERNS.md HIGH),防止持久化错误产品的数据
...
- [ ] **SEC-01**: Tauri production build 中显式声明 CSP(当前 `csp: null` 是 debt),`style-src self unsafe-inline`、`script-src` default-deny
