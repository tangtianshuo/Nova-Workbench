...
Plans:
- [ ] 15-01-PLAN.md — 存储底座：迁移 0004（memories/memory_candidates/knowledge_docs/knowledge_fts FTS5）+ ftsTokens 共享切分 + memoryStore 双实现（防轰炸三项 + supersedes 链）
- [ ] 15-02-PLAN.md — 候选流 + 上下文注入：proposeMemory tool（不终止 turn）+ 五段优先级组装器 + context_injected 审计事件 + toolLoop 接线
- [ ] 15-03-PLAN.md — 知识域落地：knowledgeRepo 双实现（版本链 + FTS5 混合检索）+ kv→SQLite 迁移 gate + rndStore 投影化 + 产品删除级联
...
| 15. 长期记忆 + 知识文档 + FTS5 检索 | 0/4 | In progress | - |
