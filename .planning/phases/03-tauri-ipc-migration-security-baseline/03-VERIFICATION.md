...
| SEC-03 | PASS (gap-closed) | **Originally**: capability file `src-tauri/capabilities/llm.json` with 4 command permission entries. **Gap discovered on first `npm run tauri:dev`**: build script rejected `generate-project:allow` etc. with `Permission not found, expected one of core:*, shell:*, sql:*...`. **Root cause**: Tauri 2.x capability permission system only governs PLUGIN commands; custom `#[tauri::command]` fns in the binary crate are auto-callable from any window with `core:default` (which `default.json` grants). **Fix**: deleted `capabilities/llm.json` entirely (commit `3960ad2`). The 4 LLM commands remain accessible — see existing `get_gnome_color_scheme` (Phase 1 custom command, no capability entry, works correctly). SEC-03 considered satisfied by `default.json`'s `core:default` grant + `invoke_handler!` registration at `lib.rs:64-67`. |
...
1. ~~**Capability identifier format unverified against live schema**~~ — **GAP CLOSED 2026-08-08 (commit `3960ad2`)**. The whole capability file was unnecessary: Tauri 2.x capability permissions only govern plugin commands, not custom `#[tauri::command]` fns. Build script rejected the file; fix was deletion. SEC-03 re-marked PASS (gap-closed).
2. **CSP `connect-src ipc: http://ipc.localhost` is critical and was missed by CONTEXT.md D-17.** RESEARCH.md §Pattern 6 corrected it. If the production build silently rejects invoke, this is now the first-place suspect (capability risk is gone).
