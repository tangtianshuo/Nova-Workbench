...
| 项 | 结果 | 备注 |
| -- | ---- | ---- |
| SC-1 | ❌ 部分失败 | SC-1.1 编辑器空白(根因已定位,见 SC-1.1 备注);SC-1.2.. 待修复后复测 |
| SC-2 | 待验 | |
| SC-3 | 待验 | |
| SC-4 | 待验 | |
...
## UAT 中段反馈(2026-09-03,待裁定进 31-07)

1. **Bug(SC-1.1)**:知识库点击文档,面板弹出但编辑器空内容 — 根因已定位(`DocWorkspaceContent.tsx` 内容采纳 effect deps 竞态)。
2. **新功能请求(用户提出)**:文档工作区支持 **tab 页打开多个文档**,切换文档时自动保存(flush 未落盘的防抖编辑)。顺带修复潜伏 bug:当前切换文档会静默丢弃 ≤800ms 未保存编辑。
