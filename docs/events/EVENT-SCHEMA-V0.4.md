...
- event: exec pid — payload 增量字段 `pid: number`(exec tool_call spawn 成功后写入 tool_result;CP-8 主层孤儿进程清杀的审计锚点)— 消费: Rust 引擎(exec.rs)+ TS 投影(可选展示)— fixture: pending-32-04 — realdb-sample(带 pid 的 exec 配对用例)
...
- event: orphan_exec_killed — 新 event_type(恢复时孤儿 exec pid 清杀审计;CP-8)— 消费: Rust restore + TS 投影(审计展示)— fixture: pending-32-04 — realdb-sample(恢复场景用例)
