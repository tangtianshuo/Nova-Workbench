...
mod tests {
    ...
    /// Cross-implementation parity: projection-cases.json was generated by the
    /// REAL TS ChatSession projection; Rust must match role+content exactly.
    #[test]
    fn projection_cases_parity_fixture() {
        let path = format!(
        ...
        assert!(cases.len() >= 8, "expected >=8 projection cases");
        for case in &cases {
            let name = case["name"].as_str().unwrap();
            let events: Vec<AgentEvent> = case["events"]
                .as_array()
                .unwrap()
                .iter()
                .map(|j| AgentEvent {
                    ...
                    workspace_id: j["workspaceId"].as_str().map(|s| s.into()),
                    product_id: j["productId"].as_str().map(|s| s.into()),
                    project_id: j["projectId"].as_str().map(|s| s.into()),
                    correlation_id: j["correlationId"].as_str().map(|s| s.into()),
                    payload: j["payload"].clone(),
                })
                .collect();
            let stream: Vec<AgentEvent> = if case["fork"].is_object() {
                ...
                assert!(built.invalid.is_none(), "case {name}: {:?}", built.invalid);
                built.events
            } else {
                events
            };
            let budget = case["budget"].as_i64().unwrap_or(DEFAULT_TOKEN_BUDGET);
            let session = ChatSession::from_events(&stream, None, Some(budget));
            let got = session.get_messages_for_llm(DEFAULT_MAX_TURNS);
            let expected = case["expectedMessages"].as_array().unwrap();
            assert_eq!(got.len(), expected.len(), "case {name}: message count");
            for (g, e) in got.iter().zip(expected) {
                assert_eq!(g.role.as_str(), e["role"].as_str().unwrap(), "case {name}: role");
                assert_eq!(g.content, e["content"].as_str().unwrap(), "case {name}: content");
            }
        }
    }
